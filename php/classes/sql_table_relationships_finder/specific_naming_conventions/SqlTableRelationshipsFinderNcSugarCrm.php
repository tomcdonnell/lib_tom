...
*
*            Also, if two table exist such that `<table_two_name>`=`<table_one_name>_cstm`, then
*            a one-to-one link between `<table_one_name>`.`id` in table one and
*           `<table_two_name>`.`id_c`.
*
*          * One-to-Many Links
...
class SqlTableRelationshipsFinderNcSugarCrm extends SqlTableRelationshipsFinder
{
   ...
   protected function getDirectLinksFromTableByTableLinkCol()
   {
      ...
      foreach ($this->columnHeadingsByTableName as $linkedTableName => $linkedTableColumnHeadings)
      {
         if
         (
            $linkedTableName == "{$this->tableName}_cstm" &&
            in_array('id_c', $linkedTableColumnHeadings)
         )
         {
            $directLinksFromTableByTableLinkCol['id'] = array
            (
               'linkedTableName'    => $linkedTableName,
               'linkedTableLinkCol' => 'id_c'
            );
         }
      }
