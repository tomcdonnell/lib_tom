...
class Utils_database
{
   ...
   (
      DatabaseConnection $dbc, $tableName, $whereDetails, $limit = null, $offset = null
   )
   {
      assert('is_string($tableName)'  );
      assert('is_array($whereDetails)');
      ...
      (
         'SELECT *
          FROM `' . mysql_real_escape_string($tableName) . '`
          WHERE ' . implode("\nAND ", $conditions) . '
         ' . (($limit  === null)? '': 'LIMIT '  . mysql_real_escape_string($limit )) . '
         ' . (($offset === null)? '': 'OFFSET ' . mysql_real_escape_string($offset))
      );
   ...
   public static function insertRowIntoTable(DatabaseConnection $dbc, $tableName, $row)
   {
      ...
      if ($dbc->getN_affectedRows() != 1)
      {
         throw new Exception('Unexpected n_affectedRows.');
      }
