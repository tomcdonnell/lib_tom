...
function SelectorHierarchical(params)
{
   ...
   this.selectOptionsFromValues = function (valueBySelectorIndex)
   {
      ...
      // NOTE
      // ----
      // The following line must precede the triggered 'change()' because the behaviour of the
      // _onChangeSelector function depends on the _state.valueToSelectWhenOptions... variable.
   ...
   function _onChangeSelector(e)
   {
      try
      {
         ...
         if
         (
            (selectorIndex == 0 && selectedOptionValue == -1) ||
            (
